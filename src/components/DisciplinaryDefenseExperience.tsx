...
export const DisciplinaryDefenseExperience: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      const before = loadDisciplinaryState(currentPlayer);
      const next = evaluateAndOpenDisciplinaryProceeding(currentPlayer, currentProfile);
      if (!active) return;
      ...
      const proceeding = next.activeProceeding;
      if (
        proceeding &&
        (proceeding.stage === 'NOTICE' || proceeding.stage === 'DEFENSE') &&
        proceeding.id !== dismissedDecisionId
      ) {
        setIsOpen(true);
      } else if (!before.activeProceeding && next.activeProceeding) {
        setIsOpen(true);
      }
    };
  ...
  const startDefense = () => {
    sound.playClick();
    const next = beginDisciplinaryDefense(player, disciplinary);
    setDisciplinary(next);
  };
  ...
  const waiveDefense = () => {
    ...
    sound.playFailure();
    const next = waiveDisciplinaryDefense(player);
    setDisciplinary(next);
  };

  const closeDecision = () => {
    if (!proceeding) return;
    setDismissedDecisionId(proceeding.id);
    setIsOpen(false);
  };
                  ...
                  <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                    ...
                    {!blocked && (
                      <button
                        type="button"
                        onClick={() => {
                          acknowledgeDisciplinaryDecision(player);
                          closeDecision();
                        }}
